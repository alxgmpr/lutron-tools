...
static const uint32_t    isr_latency_hist_max_us[CCA_ISR_LAT_BINS] = {
    10, 20, 40, 80, 160, 320, 640, 1000, 2000, 5000, 10000, 0xFFFFFFFFu
};
        ...
        for (size_t i = 0; i < rx_pend_count; i++) {
            ...
            if (p.valid) {
                ...
                n += snprintf(log_buf + n, sizeof(log_buf) - n,
                              "[cca] RX %-12s dev=%s seq=%02X rssi=%d",
                              cca_packet_type_name(p.pkt.type_byte), dev_id,
                              p.pkt.sequence, p.rssi);

                if (cca_is_button_type(p.pkt.type_byte)) {
                    n += snprintf(log_buf + n, sizeof(log_buf) - n,
                                  " btn=%s act=%s",
                                  cca_button_name(p.pkt.button),
                ...
                if (p.pkt.n81_errors > 0) {
                    n += snprintf(log_buf + n, sizeof(log_buf) - n,
                                  " n81err=%u", p.pkt.n81_errors);
                }
                const char *crc_str = p.pkt.crc_valid
                    ? (p.pkt.n81_errors > 0 ? "RCVR" : "OK") : "FAIL";
                n += snprintf(log_buf + n, sizeof(log_buf) - n,
                              " crc=%s\r\n", crc_str);
            } else {
                n += snprintf(log_buf + n, sizeof(log_buf) - n,
                              "[cca] RX DROP rssi=%d len=%u hex=",
                              p.rssi, (unsigned)p.drop_raw_len);
                for (size_t j = 0; j < p.drop_hex_len &&
                         n < (int)sizeof(log_buf) - 4; j++) {
                    n += snprintf(log_buf + n, sizeof(log_buf) - n,
                                  "%02X", p.drop_hex[j]);
                }
...
static void cca_task_func(void *param)
{
    ...
    for (;;) {
        ...
        if (notified > 0) {
            ...
            for (;;) {
                if (cc1101_check_rx()) {
                    ...
                    if (elapsed >= pdMS_TO_TICKS(CCA_DRAIN_SILENCE_MS))
                        break;
        ...
        while (xQueueReceive(cca_tx_queue, &tx_item, 0) == pdTRUE) {
            ...
            uint8_t encoded[128];
            CcaEncoder encoder;
            size_t encoded_len = encoder.encode_packet(with_crc, tx_item.len + 2,
...
uint32_t cca_rx_count(void)      { return rx_count; }
uint32_t cca_tx_count(void)      { return tx_count + cca_cmd_tx_count(); }
uint32_t cca_drop_count(void)    { return drop_count; }
uint32_t cca_crc_fail_count(void){ return crc_fail_count; }
uint32_t cca_n81_err_count(void) { return n81_err_count; }
uint32_t cca_ack_count(void)     { return ack_count; }
uint32_t cca_crc_optional_count(void) { return crc_optional_count; }
uint32_t cca_irq_count(void)     { return gdo0_irq_count; }
uint32_t cca_isr_latency_min_us(void)
{
    return (isr_latency_samples > 0) ? isr_latency_min_us : 0;
}
uint32_t cca_isr_latency_p95_us(void) { return isr_latency_p95_us_internal(); }
uint32_t cca_isr_latency_max_us(void) { return isr_latency_max_us; }
uint32_t cca_isr_latency_samples(void) { return isr_latency_samples; }
void cca_set_uart_log_enabled(bool enabled) { cca_uart_log_enabled_ = enabled; }
bool cca_uart_log_enabled(void) { return cca_uart_log_enabled_; }
...
void cca_set_rx_hook(cca_rx_hook_t hook) { rx_hook = hook; }
