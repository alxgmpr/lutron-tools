...
class CcaDecoder {
  ...
  bool decode(const uint8_t *fifo_data, size_t len, DecodedPacket &packet) {
    ...
    while (true) {
      ...
      if (decoded_len >= 10) {
        ...
        if (decoded_len > best_decoded_len ||
            (decoded_len == best_decoded_len && 0 < best_errors)) {
      ...
      if (tolerant_len >= 10 && tolerant_len > decoded_len) {
        ...
        if (errors > 0 && errors <= 2) {
          match = cca_recover_n81_errors(tolerant, tolerant_len,
                                          CCA_LENGTHS, N_CCA_LENGTHS,
                                          err_pos, errors);
          if (match > 0) {
            return parse_bytes_at_length(tolerant, static_cast<size_t>(match), errors, packet);
          }
        ...
        /* Track as candidate if better than previous best */
        if (tolerant_len > best_decoded_len ||
            (tolerant_len == best_decoded_len && errors < best_errors)) {
    ...
    if (best_decoded_len >= 10) {
      if (best_errors > 0 && best_errors <= 2) {
        int match = cca_recover_n81_errors(best_decoded, best_decoded_len,
                                        CCA_LENGTHS, N_CCA_LENGTHS,
 ...
 private:
  ...
  static int try_fast_sync(const uint8_t *data, size_t len) {
    ...
    uint8_t b1, b2, b3;
    if (n81_decode_byte(data, len, 0, &b1) &&
        n81_decode_byte(data, len, 10, &b2) &&
   ...
   */
  bool try_decode_at_offset(const uint8_t *fifo_data, size_t len,
                            size_t data_start, DecodedPacket &packet) {
    uint8_t decoded[MAX_DECODE_LEN];
    size_t decoded_len = n81_decode_stream(fifo_data, len, data_start,
                                            MAX_DECODE_LEN, decoded);
    ...
    uint8_t errors = 0;
    uint8_t err_pos[2] = {};
    size_t tolerant_len = n81_decode_stream_tolerant(fifo_data, len, data_start,
                                                      MAX_DECODE_LEN, tolerant, &errors,
                                                      err_pos, 2);
    if (tolerant_len >= 10) {
      int match = cca_check_crc_at_lengths(tolerant, tolerant_len, CCA_LENGTHS, N_CCA_LENGTHS);
