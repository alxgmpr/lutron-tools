...
bool stream_client_connected(void) { return num_clients > 0; }
int  stream_num_clients(void)      { return num_clients; }

uint32_t stream_tx_drop_count(void)  { return tx_drop_count; }
uint32_t stream_udp_sent_count(void) { return udp_sent_count; }
uint32_t stream_udp_fail_count(void) { return udp_fail_count; }
...
static void register_client(const ip_addr_t *addr, uint16_t port)
{
    ...
    for (int i = 0; i < MAX_STREAM_CLIENTS; i++) {
        if (clients[i].active &&
            ip_addr_cmp(&clients[i].addr, addr) &&
            clients[i].port == port) {
    ...
    for (int i = 0; i < MAX_STREAM_CLIENTS; i++) {
        if (!clients[i].active) {
            ...
            printf("[stream] Client %s:%u registered (%d/%d)\r\n",
                   ipaddr_ntoa(addr), port, num_clients, MAX_STREAM_CLIENTS);
...
static void expire_clients(void)
{
    ...
    for (int i = 0; i < MAX_STREAM_CLIENTS; i++) {
        if (clients[i].active &&
            (now - clients[i].last_heard) > CLIENT_TIMEOUT_MS) {
...
static void send_to_client(const uint8_t *frame, uint16_t frame_len,
                           const ip_addr_t *addr, uint16_t port)
{
    ...
    if (nb == NULL) return;
    void *p = netbuf_alloc(nb, frame_len);
    if (p == NULL) { netbuf_delete(nb); return; }
    memcpy(p, frame, frame_len);
 ...
 * ----------------------------------------------------------------------- */
static void handle_rx_data(const uint8_t *buf, size_t len,
                           const ip_addr_t *src_addr, uint16_t src_port)
{
    ...
    switch (cmd) {
        ...
        case STREAM_CMD_NRF_DFU_START: {
            if (data_len >= 4) {
                uint32_t img_size = (uint32_t)buf[2]
                                  | ((uint32_t)buf[3] << 8)
                                  | ((uint32_t)buf[4] << 16)
                                  | ((uint32_t)buf[5] << 24);
        ...
        case STREAM_CMD_CCA_BUTTON: {
            if (data_len >= 5) {
                ...
                item.device_id = ((uint32_t)buf[2] << 24) | ((uint32_t)buf[3] << 16)
        ...
        case STREAM_CMD_CCA_LEVEL: {
            if (data_len >= 10) {
                ...
                item.device_id = ((uint32_t)buf[2] << 24) | ((uint32_t)buf[3] << 16)
                               | ((uint32_t)buf[4] << 8) | buf[5];
                item.target_id = ((uint32_t)buf[6] << 24) | ((uint32_t)buf[7] << 16)
                               | ((uint32_t)buf[8] << 8) | buf[9];
                item.level_pct = buf[10];
                item.fade_qs = buf[11];
                cca_cmd_enqueue(&item);
                printf("[stream] CCA bridge level: zone=%08X target=%08X %u%%\r\n",
                       (unsigned)item.device_id, (unsigned)item.target_id, item.level_pct);
            }
            break;
        }
        ...
        case STREAM_CMD_CCA_LED: {
            if (data_len >= 9) {
                ...
                printf("[stream] CCA LED config: zone=%08X target=%08X mode=%u\r\n",
                       (unsigned)item.device_id, (unsigned)item.target_id, item.led_mode);
            }
            break;
        }
        ...
        case STREAM_CMD_CCA_PHASE: {
            if (data_len >= 9) {
                ...
                printf("[stream] CCA phase config: zone=%08X target=%08X phase=0x%02X\r\n",
                       (unsigned)item.device_id, (unsigned)item.target_id, item.phase_byte);
            }
            break;
        }
        ...
        case STREAM_CMD_CCA_PICO_PAIR: {
            if (data_len >= 6) {
                ...
                printf("[stream] CCA pico pair: dev=%08X type=%u dur=%u\r\n",
                       (unsigned)item.device_id, item.pico_type, item.duration_sec);
            }
            break;
        }
        ...
        case STREAM_CMD_CCA_BRIDGE_PAIR: {
            if (data_len >= 9) {
                ...
                printf("[stream] CCA bridge pair: bridge=%08X target=%08X beacon=%u\r\n",
                       (unsigned)item.device_id, (unsigned)item.target_id, item.duration_sec);
            }
            break;
        }
        ...
        case STREAM_CMD_CCA_VIVE_PAIR: {
            if (data_len >= 6) {
                ...
                printf("[stream] CCA vive pair: hub=%08X zone=0x%02X dur=%u\r\n",
                       (unsigned)item.device_id, item.zone_byte, item.duration_sec);
            }
            break;
        }
