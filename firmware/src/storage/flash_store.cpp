...
static const uint32_t crc32_table[256] = {
    ...
    0x65B0D9C6, 0x12B7E950, 0x8BBEB8EA, 0xFCB9887C, 0x62DD1D7F, 0x15DA2D49,
    ...
    0xA7672661, 0xD06016F7, 0x4969474D, 0x3E6E77DB, 0xAE321404, 0xD9352435,
    0x4036342E, 0x17316E58, 0xA03694BF, 0xD7316E89, 0x4C34D633, 0x1B37E6A5,
    0x6E6B5106, 0x196C6190, 0x8065302A, 0xF762004E, 0x6906C2FE, 0x1E01F268,
    0x8F0BBC72, 0xF80CE6E4, 0x68000743, 0x1F0F37D5, 0x860E6C6F, 0xF109FCF9,
...
void flash_store_init(void)
{
    ...
    if (stored->magic != FLASH_STORE_MAGIC ||
        stored->version != FLASH_STORE_VERSION) {
        printf("[flash] No valid settings (magic=0x%08lX ver=%u), loading defaults\r\n",
               (unsigned long)stored->magic, stored->version);
        load_defaults();
        return;
    }

    /* Verify CRC-32 over first 252 bytes */
    uint32_t computed = calc_crc32((const uint8_t *)stored,
                                    sizeof(FlashSettings) - sizeof(uint32_t));
    ...
    printf("[flash] Settings loaded (ch=%u panid=0x%04X devices=%u)\r\n",
           settings.thread_channel, settings.thread_panid, settings.known_count);
}

bool flash_store_save(void)
{
    /* Update CRC before writing */
    settings.crc32 = calc_crc32((const uint8_t *)&settings,
                                 sizeof(FlashSettings) - sizeof(uint32_t));
