...
static uint16_t udp_checksum(const uint8_t* src_addr, const uint8_t* dst_addr, uint16_t udp_len, const uint8_t* udp_hdr,
                             const uint8_t* payload, size_t payload_len)
{
    uint32_t sum = 0;

    /* Pseudo-header: src addr (16) + dst addr (16) + UDP length (4) + next header (4) */
    for (int i = 0; i < 16; i += 2) sum += ((uint32_t)src_addr[i] << 8) | src_addr[i + 1];
    for (int i = 0; i < 16; i += 2) sum += ((uint32_t)dst_addr[i] << 8) | dst_addr[i + 1];
    sum += udp_len;
    ...
    /* Payload */
    size_t i;
    for (i = 0; i + 1 < payload_len; i += 2) sum += ((uint32_t)payload[i] << 8) | payload[i + 1];
    if (i < payload_len) sum += (uint32_t)payload[i] << 8; /* odd byte */
                      ...
                      uint16_t src_port, uint16_t dst_port, const uint8_t* payload, size_t payload_len)
{
    ...
    if (src_addr) memcpy(pkt + 8, src_addr, 16);
    /* else: already zeroed (::) */
                              ...
                              uint16_t* dst_port, size_t* payload_len)
{
    ...
    if (payload_len) *payload_len = udp_len - UDP_HEADER_SIZE;
