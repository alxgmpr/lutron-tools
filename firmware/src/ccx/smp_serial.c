...
static size_t base64_encode(uint8_t *out, size_t out_size,
...
static size_t base64_decode(uint8_t *out, size_t out_size,
                             const uint8_t *in, size_t in_len)
{
...
static void smp_build_header(uint8_t *hdr, uint8_t op, uint16_t len,
                               uint16_t group, uint8_t seq, uint8_t id)
{
    hdr[0] = op;
    hdr[1] = 0x00;  /* flags */
    hdr[2] = (uint8_t)(len >> 8);
 ...
 * ----------------------------------------------------------------------- */
static size_t smp_wrap_frame(uint8_t *out, size_t out_size,
                              const uint8_t *smp_pkt, size_t smp_len)
{
                        ...
                        uint8_t seq)
{
    ...
    smp_build_header(smp_pkt, SMP_OP_WRITE, (uint16_t)pos,
                      SMP_GROUP_IMG, seq, SMP_ID_UPLOAD);
...
size_t smp_build_image_list(uint8_t *out, size_t out_size, uint8_t seq)
{
    ...
    uint8_t smp_pkt[16];
    smp_build_header(smp_pkt, SMP_OP_READ, (uint16_t)pos,
                      SMP_GROUP_IMG, seq, SMP_ID_STATE);
    memcpy(smp_pkt + 8, cbor, pos);
                        ...
                        int *rc, uint32_t *off)
{
    ...
    *off = 0;
    ...
    for (uint32_t i = 0; i < map_entries && remaining > 0; i++) {
        ...
        if (key_len == 2 && memcmp(key_data, "rc", 2) == 0 &&
            item.major == CBOR_MAJOR_UINT) {
            *rc = (int)item.value;
        } else if (key_len == 3 && memcmp(key_data, "off", 3) == 0 &&
                   item.major == CBOR_MAJOR_UINT) {
            *off = item.value;
        }
