...
static bool dedup_is_duplicate(uint16_t msg_type, uint8_t sequence)
{
    ...
    for (size_t i = 0; i < DEDUP_RING_SIZE; i++) {
        if (dedup_ring[i].msg_type == msg_type &&
            dedup_ring[i].sequence == sequence &&
 ...
 */
static size_t spinel_wait_response(uint8_t expect_cmd, uint8_t expect_prop,
 ...
 */
static size_t spinel_prop_get(uint8_t prop_id, uint8_t *resp, size_t resp_size,
                              uint32_t timeout_ms)
{
    ...
    return spinel_wait_response(SPINEL_CMD_PROP_IS, prop_id,
                                resp, resp_size, timeout_ms);
}
 ...
 */
static bool spinel_prop_set(uint8_t prop_id, const uint8_t *value, size_t value_len,
                             uint32_t timeout_ms)
{
    ...
    uint8_t resp[64];
    size_t len = spinel_wait_response(SPINEL_CMD_PROP_IS, prop_id,
 ...
 */
static bool spinel_prop_insert(uint8_t prop_id, const uint8_t *value, size_t value_len,
                                uint32_t timeout_ms)
{
    uint8_t frame[HDLC_RX_BUF_SIZE];
    if (3 + value_len > sizeof(frame)) return false;
    ...
    uint8_t resp[64];
    return spinel_wait_response(SPINEL_CMD_PROP_INSERTED, prop_id,
                                resp, sizeof(resp), timeout_ms) > 0;
}
...
static void dfu_enter_bootloader(void)
{
    ...
    size_t probe_len = smp_build_image_list(probe_buf, sizeof(probe_buf),
                                             smp_seq++);
    if (probe_len == 0) {
...
static void dfu_upload_loop(void)
{
    ...
    while (dfu_bytes_written < dfu_image_size && dfu_state == CCX_DFU_UPLOADING) {
        ...
        uint8_t smp_buf[512];
        size_t smp_len = smp_build_upload(smp_buf, sizeof(smp_buf),
                                           dfu_bytes_written,
                                           dfu_chunk_buf, dfu_chunk_len,
...
static bool ncp_probe(void)
{
    ...
    if (reset_len > 0) {
        printf("[ccx] NCP reset response: %u bytes, cmd=0x%02X\r\n",
               (unsigned)reset_len, reset_len >= 2 ? boot_buf[1] : 0);
    } else {
    ...
    uint8_t ver_buf[16];
    size_t ver_len = spinel_prop_get(SPINEL_PROP_PROTOCOL_VERSION,
                                     ver_buf, sizeof(ver_buf), 2000);
    if (ver_len >= 2) {
    ...
    uint8_t ncp_ver[128];
    size_t ncp_len = spinel_prop_get(SPINEL_PROP_NCP_VERSION,
                                     ncp_ver, sizeof(ncp_ver) - 1, 2000);
    if (ncp_len > 0) {
        ncp_ver[ncp_len] = '\0';
    ...
    /* Query EUI-64 */
    uint8_t hwaddr[8];
    size_t hw_len = spinel_prop_get(SPINEL_PROP_HWADDR,
                                    hwaddr, sizeof(hwaddr), 2000);
    if (hw_len == 8) {
...
static bool thread_join(void)
{
    printf("[ccx] Joining Lutron Thread network (ch=%d, PAN=0x%04X)...\r\n",
           LUTRON_THREAD_CHANNEL, LUTRON_THREAD_PANID);
    ...
    uint8_t panid[2] = {
        (uint8_t)(LUTRON_THREAD_PANID & 0xFF),
        (uint8_t)(LUTRON_THREAD_PANID >> 8)
    };
    ...
    if (!spinel_prop_set(SPINEL_PROP_NET_XPANID,
                         LUTRON_THREAD_XPANID, 8, 2000)) {
    ...
    if (!spinel_prop_set(SPINEL_PROP_NET_NETWORK_KEY,
                         LUTRON_THREAD_MASTER_KEY, 16, 2000)) {
    ...
    for (int attempt = 0; attempt < 60; attempt++) {
        ...
        uint8_t role_buf[4];
        size_t role_len = spinel_prop_get(SPINEL_PROP_NET_ROLE,
                                          role_buf, sizeof(role_buf), 1000);
        if (role_len >= 1) {
    ...
    if (!spinel_prop_insert(SPINEL_PROP_IPV6_MULTICAST_ADDRESS_TABLE,
                             CCX_MULTICAST_ADDR, 16, 2000)) {
    ...
    size_t addr_len = spinel_prop_get(SPINEL_PROP_IPV6_ADDRESS_TABLE,
                                      addr_buf, sizeof(addr_buf), 2000);
    if (addr_len > 0) {
...
static void ccx_process_tx(const ccx_tx_request_t *req)
{
    ...
    if (req->type == 0) {
        cbor_len = ccx_encode_level_control(cbor_buf, sizeof(cbor_buf),
                                             req->id, req->level,
                                             req->fade, req->sequence);
    } else if (req->type == 1) {
        cbor_len = ccx_encode_scene_recall(cbor_buf, sizeof(cbor_buf),
                                            req->id, req->sequence);
    }
...
static void ccx_process_rx(const uint8_t *spinel_payload, size_t payload_len)
{
    ...
    const uint8_t *udp_data = ipv6_udp_parse(spinel_payload, payload_len,
                                              src_addr, &src_port, &dst_port,
                                              &udp_payload_len);
    if (!udp_data) return;
        ...
        switch (msg.msg_type) {
            ...
            case CCX_MSG_BUTTON_PRESS:
                printf("[ccx] RX %s dev=%02X%02X%02X%02X seq=%u\r\n",
                       type_name,
                       msg.device_id[0], msg.device_id[1],
                       msg.device_id[2], msg.device_id[3],
                       msg.sequence);
                break;
            ...
            case CCX_MSG_DIM_STEP:
                printf("[ccx] RX %s dev=%02X%02X%02X%02X action=%u step=%u seq=%u\r\n",
                       ...
                       msg.device_id[0], msg.device_id[1],
                       msg.device_id[2], msg.device_id[3],
                       msg.action, msg.step_value, msg.sequence);
                break;
            case CCX_MSG_ACK:
                printf("[ccx] RX %s response=%02X seq=%u\r\n",
                       type_name,
                       msg.response_len > 0 ? msg.response[0] : 0,
                       msg.sequence);
                break;
            ...
            case CCX_MSG_STATUS:
                printf("[ccx] RX %s dev=%02X%02X%02X%02X seq=%u\r\n",
                       type_name,
                       (unsigned)((msg.device_serial >> 24) & 0xFF),
                       (unsigned)((msg.device_serial >> 16) & 0xFF),
                       (unsigned)((msg.device_serial >> 8) & 0xFF),
                       (unsigned)(msg.device_serial & 0xFF),
                       msg.sequence);
                break;
...
static void shell_cmd_process(void)
{
    ...
    case CCX_SPINEL_PROP_GET: {
        size_t len = spinel_prop_get(shell_cmd_req.prop_id,
                                      shell_cmd_resp.data,
                                      sizeof(shell_cmd_resp.data), 3000);
        if (len > 0) {
    ...
    case CCX_SPINEL_PROP_SET: {
        bool ok = spinel_prop_set(shell_cmd_req.prop_id,
                                   shell_cmd_req.value,
                                   shell_cmd_req.value_len, 3000);
        shell_cmd_resp.success = ok;
        break;
    }
    case CCX_SPINEL_PROP_INSERT: {
        bool ok = spinel_prop_insert(shell_cmd_req.prop_id,
                                      shell_cmd_req.value,
                                      shell_cmd_req.value_len, 3000);
        shell_cmd_resp.success = ok;
        break;
    ...
    case CCX_SPINEL_RAW: {
        ...
        size_t len = hdlc_recv_frame(shell_cmd_resp.data,
                                      sizeof(shell_cmd_resp.data), 3000);
        if (len > 0) {
            shell_cmd_resp.success = true;
            shell_cmd_resp.data_len = len;
    ...
    for (;;) {
        ...
        if (ncp_detected) {
            ...
            if (rx_len >= 3) {
                ...
                if ((cmd == SPINEL_CMD_PROP_INSERTED || cmd == SPINEL_CMD_PROP_IS) &&
                    prop == SPINEL_PROP_STREAM_NET) {
...
uint32_t ccx_rx_count(void) { return rx_count; }
uint32_t ccx_tx_count(void) { return tx_count; }
uint8_t ccx_thread_role_id(void) { return thread_role; }
