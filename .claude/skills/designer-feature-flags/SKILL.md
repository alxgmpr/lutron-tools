...
For actual patch implementation see `~/lutron-security/exploits/designer-jailbreak/dll-patcher/DllPatcher/Program.cs` (dnlib-based — do not hand-edit IL bytes). For deploy, use `anthropic-skills:designer-deploy`.
...
Don't hand-edit IL bytes. All patches go through `~/lutron-security/exploits/designer-jailbreak/dll-patcher/DllPatcher/Program.cs` (dnlib-based). To add a new patch:
