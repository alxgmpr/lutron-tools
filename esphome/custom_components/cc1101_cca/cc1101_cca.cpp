...
void CC1101CCA::send_button_hold(uint32_t device_id, uint8_t button, uint16_t duration_ms) {
  // Hold-to-dim: burst of HOLD press packets, wait for duration, then burst release packets.
  // Dimmer starts dimming on press burst and stops on release burst.
  // No need to continuously emit — just press, wait, release.
  ...
  // --- PRESS BURST: 14 long-format HOLD packets ---
  ...
  ESP_LOGI(TAG, "Hold press burst sent, waiting %dms...", duration_ms);

  // --- WAIT for hold duration ---
  delay(duration_ms);

  // --- RELEASE BURST: 14 long-format RELEASE packets ---
  for (int rep = 0; rep < 14; rep++) {
    memset(packet, 0xCC, sizeof(packet));

    packet[0] = type_base | 0x01;  // LONG format (same type group as press)
    packet[1] = seq;
    packet[2] = (device_id >> 24) & 0xFF;
    packet[3] = (device_id >> 16) & 0xFF;
    packet[4] = (device_id >> 8) & 0xFF;
  ...
  ESP_LOGI(TAG, "=== HOLD COMPLETE (press + %dms + release) ===", duration_ms);
}
