...
const TS_OUTPUT_PATH = join(import.meta.dir, "../protocol/generated/typescript/protocol.ts");
const CPP_OUTPUT_PATH = join(import.meta.dir, "../firmware/src/cca/cca_types.h");
...
function generateTS(def: ProtocolDef) {
  ...
  // Meta & Ecosystems
  out += "export const ECOSYSTEMS = " + JSON.stringify(def.meta.ecosystems, null, 2) + " as const;\n\n";
  out += "export const ID_SCHEMAS = " + JSON.stringify(def.id_schemas, null, 2) + " as const;\n\n";
  ...
  for (const [name, enumDef] of Object.entries(def.enums)) {
    ...
    if (hasValues) {
        const sortedValues = entries.sort((a: any, b: any) => (a[1].value || 0) - (b[1].value || 0));
        for (const [k, v] of sortedValues as any) {
          if (v.description) out += "  /** " + v.description + " */\n";
          out += "  " + k + ": " + v.value + ",\n";
        }
        out += "} as const;\n\n";
        out += "export type " + enumName + " = typeof " + enumName + "[keyof typeof " + enumName + "];\n\n";
  ...
  const sortedTypes = Object.entries(def.packet_types).sort((a: any, b: any) => (a[1] as any).value - (b[1] as any).value);
  for (const [k, v] of sortedTypes as any) {
    if (v.description) out += "  /** " + v.description + " */\n";
    out += "  " + k + ": " + v.value + ",\n";
  }
  out += "} as const;\n\n";
  out += "export type PacketType = typeof PacketType[keyof typeof PacketType];\n\n";
  ...
  for (const [k, v] of sortedTypes as any) {
    ...
    out += "    usesBigEndianDeviceId: " + (v.device_id_endian === "big") + ",\n";
  ...
  out += "export type FieldFormat = 'hex' | 'decimal' | 'device_id' | 'device_id_be' | 'level_byte' | 'level_16bit' | 'button' | 'action';\n\n";
  ...
  for (const [k, v] of Object.entries(def.sequences) as any) {
    ...
    for (const s of v.steps) {
      out += "      { packetType: '" + s.packet + "', count: " + s.count + ", intervalMs: " + (s.interval_ms || 0) + " },\n";
    }
    out += "    ],\n";
    out += "  },\n";
  }
  ...
  out += "export function isPacketCategory(typeCode: number, category: string): boolean {\n";
...
function generateCPP(def: ProtocolDef) {
  ...
  out += "static const size_t CCA_PKT_STANDARD_LEN = " + def.lengths.standard + ";\n";
  out += "static const size_t CCA_PKT_PAIRING_LEN = " + def.lengths.pairing + ";\n\n";
  ...
  for (const [k, v] of Object.entries(def.packet_types) as any) {
    ...
    out += "static const uint8_t " + name + " = 0x" + v.value.toString(16).toUpperCase() + ";\n";
    if (k === "SET_LEVEL") {
  ...
  for (const [k, v] of Object.entries(def.enums.button.values) as any) {
    out += "static const uint8_t BTN_" + k + " = 0x" + v.value.toString(16).toUpperCase() + ";\n";
  }

  out += "\n// Action codes\n";
  for (const [k, v] of Object.entries(def.enums.action.values) as any) {
    out += "static const uint8_t ACTION_" + k + " = 0x" + v.value.toString(16).toUpperCase() + ";\n";
  }
  ...
  out += "// CRC polynomial\n";
  out += "static const uint16_t LUTRON_CRC_POLY = 0x" + def.crc.polynomial.toString(16).toUpperCase() + ";\n\n";

  out += "#ifdef __cplusplus\n}\n#endif\n\n#ifdef __cplusplus\n\n";

  out += "// Get human-readable packet type name\ninline const char *cca_packet_type_name(uint8_t type_byte) {\n  switch (type_byte) {\n";
  for (const [k, v] of Object.entries(def.packet_types) as any) {
    out += "    case 0x" + v.value.toString(16).toUpperCase() + ": return \"" + k + "\";\n";
  }
  out += "    default: return \"UNKNOWN\";\n  }\n}\n\n";

  out += "// Get expected packet length for a type byte.\ninline int cca_get_packet_length(uint8_t type_byte) {\n";
  ...
  out += "  if (type_byte >= 0xC0 && type_byte <= 0xEF) return 24;\n  return 0;\n}\n\n";

  out += "// Check if type byte is a button press\ninline bool cca_is_button_type(uint8_t type_byte) {\n";
  out += "  return type_byte >= 0x88 && type_byte <= 0x8B;\n}\n\n";

  out += "// Check if type byte is a pairing announcement\ninline bool cca_is_pairing_type(uint8_t type_byte) {\n";
  out += "  return type_byte == 0xB0 || type_byte == 0xB8 || type_byte == 0xB9 ||\n         type_byte == 0xBA || type_byte == 0xBB;\n}\n\n";

  out += "// Check if type byte is a handshake\ninline bool cca_is_handshake_type(uint8_t type_byte) {\n";
  out += "  return (type_byte >= 0xC1 && type_byte <= 0xE0 && type_byte != 0xC0);\n}\n\n";

  out += "// Check if type byte uses big-endian device ID\ninline bool cca_uses_be_device_id(uint8_t type_byte) {\n  switch (type_byte) {\n";
  for (const [k, v] of Object.entries(def.packet_types) as any) {
...
try {
  ...
  mkdirSync(join(import.meta.dir, "../protocol/generated/typescript"), { recursive: true });
