...
echo "  npx tsx tools/nrf-dfu-flash.ts --tmf        # TMF-extension build"
echo "  npx tsx tools/nrf-dfu-flash.ts --rollback   # known-good baseline"
echo ""
echo "Flash via Nucleo TCP stream (legacy/STM32-relayed path):"
echo "  arm-none-eabi-objcopy -O binary $ELF /tmp/ot-ncp-ftd-nucleo-tmf.bin"
echo "  npx tsx tools/nrf-dfu-stream-flash.ts /tmp/ot-ncp-ftd-nucleo-tmf.bin --host \$NUCLEO_HOST"
