...
const delaySec = parseInt(process.argv[3] || "8");
...
async function sendRaw(desc: string, b17: number, b18: number, b19: number, b20: number, b21: number) {
  const hex = [b17, b18, b19, b20, b21].map(b => b.toString(16).padStart(2, "0")).join(" ");
...
async function main() {
  ...
  for (let i = 0; i < permutations.length; i++) {
    ...
    await sendRaw(`[${i + 1}/${permutations.length}] ${desc}`, ...bytes as [number, number, number, number, number]);
