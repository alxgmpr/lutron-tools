...
const EXPECTED_DIFF = new Set([
  0, 1, 2, 3, 4, 5, 9, 10, 11, 12, 20, 21, 51, 52,
]);
...
// ── Our TX packets ──
const ourTx = parseHex(
  "A101AD01100021150006FDEF87FE065000020813983F230B600000CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC6929",
);
...
const realHighNonShifted = parseHex(
  "a2 02 af 90 2c 00 21 15 00 07 03 c3 c6 fe 06 50 00 02 08 13 ef 35 23 0b",
);
const realHighShifted = parseHex(
  "90 2c 00 21 15 00 07 03 c3 c6 fe 06 50 00 02 08 13 ef 35 23 0b 60 00 00",
);
...
const realLowNonShifted = parseHex(
  "a2 02 af 90 2c 00 21 15 00 07 03 c3 c6 fe 06 50 00 02 08 13 ef 38 23 0b",
);
const realLowShifted = parseHex(
  "90 2c 00 21 15 00 07 03 c3 c6 fe 06 50 00 02 08 13 ef 38 23 0b 60 00 00",
);
...
function compare(
  ...
  for (let i = 0; i < Math.min(maxLen, 53); i++) {
    ...
    if (i >= theirs.length) {
      ...
      match = "  ~ (expected diff)";
      diffs.push(
        `  b${i.toString().padStart(2)} ${field.padEnd(10)} ${ourByte} vs ${theirByte} (expected: different device/zone/seq)`,
      );
    } else {
      match = "  ✗ UNEXPECTED";
      unexpectedDiffs.push(
        `  b${i.toString().padStart(2)} ${field.padEnd(10)} ${ourByte} vs ${theirByte}`,
      );
    }
    ...
    if (
      ...
      i >= 51 ||
      ours[i] !== 0xcc ||
      (i < theirs.length && theirs[i] !== 0xcc)
    ) {
      console.log(
        `  ${i.toString().padStart(2)}   ${field.padEnd(12)} 0x${ourByte}   0x${theirByte}   ${match}`,
      );
    }
  }
...
// Run comparisons
compare(
  "Our TX vs Real Bridge HIGH-END trim",
  ourTx,
  realHigh,
  "entire-trim-high-end",
);
compare(
  "Our TX vs Real Bridge LOW-END trim",
  ourTx,
  realLow,
  "entire-trim-low-end",
);
...
console.log(
  "Byte  Field       " + reals.map((r) => r.label.padEnd(18)).join(""),
);
console.log(
  "────  ──────────  " + reals.map(() => "──────────────────").join(""),
);
for (let i = 13; i <= 26; i++) {
  ...
  const values = reals.map((r) =>
    i < r.data.length ? `0x${hex(r.data[i])}`.padEnd(18) : "--".padEnd(18),
  );
  console.log(
    `  ${i.toString().padStart(2)}   ${field.padEnd(12)}${values.join("")}`,
  );
}
...
for (let i = 13; i <= 26; i++) {
  const field = FIELD_MAP[i] || `b${i}`;
  console.log(
    `  ${i.toString().padStart(2)}   ${field.padEnd(12)}0x${hex(ourTx[i])}`,
  );
}
