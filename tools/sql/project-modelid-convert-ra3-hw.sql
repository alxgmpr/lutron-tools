...
/*
Model-ID-only conversion.
Does NOT update tblProject/tblVersion/tblVersionHistory ProductType.

Use this when you want to switch hardware model identities (RA3 <-> HW style)
without changing project-level system mode markers.

Safety:
- This script only writes to tables in the CURRENT database (dbo.*).
- It reads SQLMODELINFO as lookup only.
*/

DECLARE @Direction NVARCHAR(16) = N'RA3_TO_HW'; -- RA3_TO_HW | HW_TO_RA3
DECLARE @DryRun BIT = 1;                        -- 1 = preview only
DECLARE @FailOnUnmapped BIT = 1;                -- 1 = abort if any model does not map
...
-- Package-level exception where direct HQR kit string is not present.
...
IF @DryRun = 1
BEGIN
  SELECT N'DRY_RUN_ONLY' AS Status, @Direction AS Direction, @ModelDbName AS ModelInfoDatabase;
  RETURN;
END;
...
BEGIN TRY
  ...
  INSERT INTO #UpdateStats (TableName, UpdatedRows)
  VALUES (N'tblProject (NeedsSave)', @@ROWCOUNT);
