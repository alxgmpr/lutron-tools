...
console.log(`\n${"=".repeat(80)}`);
console.log(
  `CCA SESSION ANALYSIS — ${files.length} files in ${groups.size} groups`,
);
...
for (const [name, packets] of [...groupData.entries()].sort()) {
  ...
  for (const [type, pkts] of [...byType.entries()].sort()) {
    console.log(
      `  Type 0x${type}: ${pkts.length} packets, len=${pkts[0].length}`,
    );
    // Show first packet
...
function getRepresentative(name: string, typeByte?: string): string[] | null {
  ...
  if (typeByte) {
    const match = pkts.find(
      (p) => p[0]?.toLowerCase() === typeByte.toLowerCase(),
    );
    return match || null;
  }
  return pkts[0];
...
const dominantType = [...allTypeCounts.entries()].sort(
  (a, b) => b[1] - a[1],
)[0]?.[0];
if (dominantType) {
  console.log(
  ...
  if (reps.length > 0) {
    ...
    console.log(
      `${"----".padEnd(6)} ${reps.map(() => "-".repeat(14)).join(" ")}`,
    );
...
for (const name of holdGroups) {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    console.log(
      `  pkt[${String(i).padStart(2)}] type=0x${bytes[0]?.toUpperCase()} seq=${parseInt(bytes[1], 16).toString().padStart(3)} | ${bytes.map((b) => b.toUpperCase()).join(" ")}`,
    );
  }
}
