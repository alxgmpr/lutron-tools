...
async function main() {
  ...
  console.log(`  Duration: ${duration === 0 ? 'indefinite (Ctrl+C to stop)' : `${duration} seconds`}`);
    ...
    try {
      ...
      for (const pkt of packets) {
        ...
        if (pktType !== lastType) {
          ...
          console.log(`[${elapsed}s] ${dir} ${pktType}${rssi}: ${hexPreview}...`);
          lastType = pktType;
        } else {
          // Same type - just show count on same line
          process.stdout.write(`\r[${elapsed}s] ${dir} ${pktType} x${typeCounts[pktType]}${rssi}      `);
        }
      }
    } catch (err) {
