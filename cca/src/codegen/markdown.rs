...
fn generate_packet_types_summary(out: &mut String, protocol: &Protocol) {
    ...
    for (name, pkt) in &protocol.packet_types {
        let aliases = if pkt.aliases.is_empty() {
            ...
            let alias_strs: Vec<_> = pkt.aliases.iter().map(|a| format!("0x{:02X}", a)).collect();
...
fn generate_sequence_doc(
    out: &mut String,
    name: &str,
    seq: &super::SequenceDef,
    protocol: &Protocol,
) {
    out.push_str(&format!("### {}\n\n", to_title_case(name)));
    out.push_str(&format!("{}\n\n", seq.description));
    ...
    let total_packets: u32 = seq.steps.iter().filter_map(|s| s.count).sum();
    let has_infinite = seq.steps.iter().any(|s| s.count.is_none());

    if total_packets > 0 && !has_infinite {
        let total_time: u32 = seq
            .steps
            .iter()
            .filter_map(|s| {
                s.count
                    .map(|c| c * s.interval_ms.unwrap_or(protocol.timing.button_repeat_ms))
            })
            .sum();
        out.push_str(&format!(
            "**Total:** {} packets, ~{} ms\n\n",
