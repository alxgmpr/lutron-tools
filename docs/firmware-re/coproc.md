...
**Extraction tool**: `~/lutron-security/exploits/firmware-unlock/coproc-extract.py`
...
- **RA2 Select REP2 / Caseta Pro** (1.5 MB binary): same continuous cipher, but
  ...
  `~/lutron-security/exploits/firmware-unlock/coproc-extract.py`'s `extract_multi_continuous_blobs()` walker scans the
...
- `~/lutron-security/exploits/firmware-unlock/coproc-extract.py` — S19 extraction/deobfuscation tool
