...
**Captured from real Vive Hub (017D5363) pairing PowPak (021AD0C3):**
...
**Pairing flow:**
1. Call `start_vive_pairing(0x017D5363)` - devices flash
...
```bash
# Start pairing mode
curl -X POST http://localhost:5001/api/vive/start -d '{"hub_id":"0x017D5363"}'

# Stop pairing mode
curl -X POST http://localhost:5001/api/vive/stop

# Manual beacon burst
curl -X POST http://localhost:5001/api/vive/beacon -d '{"hub_id":"0x017D5363","count":9}'

# Manual accept (if auto-accept disabled)
curl -X POST http://localhost:5001/api/vive/accept -d '{"hub_id":"0x017D5363","device_id":"0x021AD0C3"}'
```
