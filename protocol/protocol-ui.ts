...
export function identifyPacket(data: Uint8Array | number[]): IdentifiedPacket {
  ...
  if (virtualRules && formatByte !== undefined) {
    ...
    if (virtualName) {
      ...
      const virtualInfo = Object.values(PacketTypeInfo).find(i => i.name === virtualName)
...
export function getButtonName(code: number): string {
  return (ButtonNames as Record<number, string>)[code] ?? `0x${code.toString(16).toUpperCase().padStart(2, '0')}`
}
...
export function getActionName(code: number): string {
  return (ActionNames as Record<number, string>)[code] ?? `0x${code.toString(16).toUpperCase().padStart(2, '0')}`
}
...
export function parseFieldValue(
  ...
  switch (format) {
    ...
    case 'hex':
      ...
      if (fieldBytes.length === 5 && fieldBytes.every(b => b.toUpperCase() === 'FF')) {
        decoded = 'BROADCAST'
      }
      break
  }
